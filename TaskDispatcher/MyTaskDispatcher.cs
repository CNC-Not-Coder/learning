...
namespace TaskDispatcher
{
    public class MyTaskDispatcher
    {
        ...
        public MyTaskDispatcher(int threadNum, bool isPassive)
        {
            if (threadNum < 1)
                threadNum = 1;
            m_ThreadNum = threadNum;
        ...
        private void InitTaskThreads(int threadNum, bool isPassive)
        {
            if (isPassive)
            {
                m_ActionQueue = new AsyncActionProcessor();
            }
            ...
            for (int i = 0; i < threadNum; ++i)
            {
                ...
                if (isPassive)
                {
                    thread = new MyThread(m_ActionQueue);//线程主动取策略
                }
                else
                {
                    thread = new MyThread();//dispatcher主动推策略
                }
                m_Threads[i] = thread;
                thread.Start();
            }
