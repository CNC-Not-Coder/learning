...
namespace SkillFramework
{
    public sealed class SkillSection
    {
        ...
        public void Load(FunctionData sectionData, int skillId)
        {
            CallData callData = sectionData.Call;
            if (null != callData && callData.HaveParam())
            {
                ...
                {
                    m_Duration = 1000;
                    Logger.Error("Skill {0} DSL, section must have a time argument !", skillId);
                }
            }
    ...
    public sealed class SkillInstance
    {
        ...
        public float MaxMoveDeltaX
        {
            get { return m_MaxMoveDeltaX; }
            set { m_MaxMoveDeltaX = value; }
        }
        public float MaxMoveDeltaZ
        {
            get { return m_MaxMoveDeltaZ; }
            set { m_MaxMoveDeltaZ = value; }
        }
        ...
        public bool Init(ScriptableDataInfo config)
        {
            bool ret = false;
                ...
                foreach (ISyntaxComponent info in skill.Statements)
                {
                    if (info.GetId() == "section")
                    {
                        ...
                        if (null != sectionData)
                        {
                            ...
                            m_TotalTime += section.Duration;
                        }
                        else
                        {
                            Logger.Error("Skill {0} DSL, section must be a function !", m_SkillId);
                        }
                    }
                    else if (info.GetId() == "onmessage")
                    {
                        ...
                        {
                            Logger.Error("Skill {0} DSL, onmessage must be a function !", m_SkillId);
                        }
                    }
                    else if (info.GetId() == "onstop")
                    {
                        ...
                        {
                            Logger.Error("Skill {0} DSL, onstop must be a function !", m_SkillId);
                        }
                    }
                    else if (info.GetId() == "oninterrupt")
                    {
                        ...
                        {
                            Logger.Error("Skill {0} DSL, oninterrupt must be a function !", m_SkillId);
                        }
                    }
                    else
                    {
                        Logger.Error("SkillInstance::Init, unknown part {0}", info.GetId());
                    }
                }
            }
            else
        ...
        private float m_MaxMoveDeltaX = 0;
        private float m_MaxMoveDeltaZ = 0;
